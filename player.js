...
const musicIcons = require('./UI/icons/musicicons.js'); // Assuming this path is correct
    ...
    client.riffy.on("trackStart", async (player, track) => {
        ...
        try {
            const musicard = await Dynamic({
                ...
                backgroundImage: track.info.thumbnail,
                progress: player.position / track.info.length * 100 || 65,
                progressColor: '#00DFFF',
            ...
            .setFooter({ text: `Developed by Nafisa`, iconURL: musicIcons.heartIcon })
            .setTimestamp()
            .setDescription(  
...
function createActionRow1(disabled) {
    return new ActionRowBuilder()
        .addComponents(
            // Updated to use custom musicIcons
            new ButtonBuilder().setCustomId("loopToggle").setEmoji(musicIcons.loopToggle).setLabel("Loop").setStyle(ButtonStyle.Secondary).setDisabled(disabled),
            new ButtonBuilder().setCustomId("disableLoop").setEmoji(musicIcons.disableLoop).setLabel("No Loop").setStyle(ButtonStyle.Secondary).setDisabled(disabled),
            new ButtonBuilder().setCustomId("skipTrack").setEmoji(musicIcons.skipTrack).setLabel("Skip").setStyle(ButtonStyle.Secondary).setDisabled(disabled),
            new ButtonBuilder().setCustomId("showLyrics").setEmoji(musicIcons.showLyrics).setLabel("Lyrics ").setStyle(ButtonStyle.Secondary).setDisabled(disabled),
            new ButtonBuilder().setCustomId("clearQueue").setEmoji(musicIcons.clearQueue).setLabel("Clear").setStyle(ButtonStyle.Secondary).setDisabled(disabled)
        );
}
...
function createActionRow2(disabled) {
    return new ActionRowBuilder()
        .addComponents(
            // Updated to use custom musicIcons
            new ButtonBuilder().setCustomId("stopTrack").setEmoji(musicIcons.stopTrack).setLabel("Stop").setStyle(ButtonStyle.Danger).setDisabled(disabled),
            new ButtonBuilder().setCustomId("pauseTrack").setEmoji(musicIcons.pauseTrack).setLabel("Pause").setStyle(ButtonStyle.Secondary).setDisabled(disabled),
            new ButtonBuilder().setCustomId("resumeTrack").setEmoji(musicIcons.resumeTrack).setLabel("Resume").setStyle(ButtonStyle.Secondary).setDisabled(disabled),
            new ButtonBuilder().setCustomId("volumeUp").setEmoji(musicIcons.volumeUp).setLabel("Vol +").setStyle(ButtonStyle.Secondary).setDisabled(disabled),
            new ButtonBuilder().setCustomId("volumeDown").setEmoji(musicIcons.volumeDown).setLabel(" Vol - ").setStyle(ButtonStyle.Secondary).setDisabled(disabled)
        );
}
